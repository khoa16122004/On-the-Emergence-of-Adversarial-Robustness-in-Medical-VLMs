    ...
    for model_name in os.listdir(args.transfer_dir):
        ...
        for index in tqdm(indxs):
            ...
            if args.mode == "post_transform": # knowing transform
                ...
                noise = torch.randn_like(img_attack_tensor).cuda()
                img_attack_tensor = (img_attack_tensor + args.epsilon * noise).clamp(0, 1)
                # save_image(
                #     img_attack_tensor,
                #     'noisy.png'
                # )
                # img_attack_tensor = denoiser(img_attack_tensor)
                # save_image(
                #     img_attack_tensor,
                #     'denoiser.png'
                # )
                ...
                # img_attack_tensor = _toTensor(adv_img).unsqueeze(0).cuda()
                img_attack_pil = to_pil_image(
                    img_attack_tensor.detach().cpu().clamp(0, 1).squeeze(0)
                )
                img_attack_pil = SIZE_TRANSFORM[args.model_name](img_attack_pil)
                img_attack_tensor = _toTensor(img_attack_pil).unsqueeze(0).cuda()
                noise = torch.randn_like(img_attack_tensor).cuda()
                img_attack_tensor = (img_attack_tensor + args.epsilon * noise).clamp(0, 1)
                # save_image(
                #     img_attack_tensor,
...
def get_args():
    ...
    parser.add_argument("--pretrained_denoiser", type=str)
    # transfer_path
...
# thay thế ảnh gốc: có task sau
# Adversarial Training Trên Vision only với quy trình giống SSL, xong vẫn contrastive learning.